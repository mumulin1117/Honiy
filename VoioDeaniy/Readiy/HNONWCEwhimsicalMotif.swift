...
public class HNONWCEwhimsicalMotif: NSObject {

    // MARK: - 1. 单例
     static let shared = HNONWCEwhimsicalMotif()
    
    // MARK: - 暴露配置类
    public var APPPREFIX_config: TgextileLuster {
        return TgextileLuster.shared
    }
    ...
    @objc public func APPPREFIX_didRegisterForRemoteNotifications(deviceToken: Data) {
        ...
        let APPPREFIX_pushtoken = deviceToken.map { String(format: HNONWCcasualChic.APPPREFIX_1, $0) }.joined()
  
        UserDefaults.standard.set(APPPREFIX_pushtoken, forKey: HNONWCcasualChic.APPPREFIX_61)
